Watchtower is itself packaged as a Docker container so installation is as simple as pulling the `falconnl93/watchtower` image. If you are using ARM based architecture, pull the appropriate `falconnl93/watchtower:armhf-<tag>` image from the [falconnl93 Docker Hub](https://hub.docker.com/r/falconnl93/watchtower/tags/).
...
docker run -d \
  ...
  -v /var/run/docker.sock:/var/run/docker.sock \
  falconnl93/watchtower container_to_watch --debug
```
